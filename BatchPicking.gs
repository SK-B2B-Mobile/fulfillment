...
function setPackingMoved(data) {
  ...
  //   건드렸음(신규 발견 버그). 패킹존 이동 상태를 바꾸면 상세조회 결과의
  //   movedToPacking/packStage 필드가 최대 30초간 예전 값으로 보일 수 있었던
  //   잠복 버그를 여기서 같이 고침. syncInspectionFromPicking_과 마찬가지로
...
function logIssue(data) {
  ...
  try {
    ...
    // ★ 세션C 신규 — 시트와 Firestore(아래)에 정확히 동일한 시각을 쓰기 위해
    //   batchNow_()를 한 번만 계산해서 재사용(예전엔 두 setValues에서 각각 따로
    //   호출해서, 이론상 몇 밀리초 차이가 날 수 있었음 — 감사기록 정확성을 위해 통일).
    const nowStr = batchNow_();
    ...
    il.getRange(ilNewRow, 1, 1, 13).setValues([[
      data.batchId, issueId, nowStr, data.worker || '',
    ...
    sl2.getRange(sl2NewRow, 1, 1, 12).setValues([[
      data.batchId, 'ADJ-' + issueId, nowStr, data.worker || '',
    ...
    //   호출하지 않음(외부 API 호출로 락을 붙잡고 있으면 다른 작업자의 스캔·
    //   이슈등록이 전부 그 뒤에서 기다려야 하므로 — logScan/logIssue의 기존
    //   성능수정과 동일한 원칙). 락 해제 후 아래에서 best-effort로 씀.
    issueDocForFirestore = {
      batchId: data.batchId, issueId: issueId, timestamp: nowStr, worker: data.worker || '',
      barcode: data.barcode || '', sku: data.sku || '', name: data.name || '',
      invoice: data.invoice, customer: data.customer || '', reason: data.reason || 'ETC',
      qty: qty, note: data.note || '', status: 'active',
    };
  ...
  if (needsSync) {
    ...
    // ★ 세션A 신규 — 예전엔 logIssue가 salesInvDetail 캐시/미러를 전혀 안
    //   건드렸음(신규 발견 버그). 이슈 등록 직후 상세조회를 열면 최대 6초간
    //   예전 이슈 목록/상태가 보일 수 있었던 잠복 버그를 여기서 같이 고침.
    //   syncInspectionFromPicking_과 마찬가지로 락 밖에서, best-effort로 처리.
    try { syncSalesInvoiceDetailMirror_(data.invoice); } catch (eMirror) { /* 무시 */ }
    // ★ 세션C 신규 — "①번 이중쓰기" 설계의 첫 적용. 구글시트(진짜 데이터)에는
    //   이미 성공적으로 쓴 뒤이므로, 여기서 실패해도 이슈 등록 결과 자체엔
    //   전혀 영향이 없음(호출부인 board.html/batch.html은 이 실패를 알 수도
    //   없고 알 필요도 없음 — 완전한 best-effort).
...
function _buildBatchAggregates_(batchId) {
  ...
  try {
    ...
    // ★ 2026-09-24 재수정 — 처음엔 6초, 그다음 60초로 시도했는데 둘 다 문제가
    //   있었음:
    //   · 6초 = board.html 폴링 주기(30초)보다 훨씬 짧아서, 폴링 1번이 지나갈
    //     때마다 캐시가 이미 죽어있어 슬롯 클릭이 거의 항상 캐시 미스(느림).
    //   · 60초 = 폴링 주기(30초)의 2배라서, TV가 연속 2번 폴링해도 "같은 값"을
    //     또 보여줄 수 있음 — 현장 작업자가 스캔 직후 TV를 보는데 진행률이
    //     최대 60초까지 안 바뀔 수 있는 위험한 회귀였음(작업 현장 안전과
    //     직결되는 부분이라 절대 안 됨).
    //   → 정답은 "TV 폴링 주기와 정확히 맞추되 살짝 못 미치게(25초)". 이러면:
    //   1) board.html이 30초마다 폴링할 때, 직전 폴링이 남긴 캐시(25초 TTL)는
    //      항상 이미 만료된 상태 → 매 폴링마다 100% 새로 계산 → TV 화면은
    //      지금까지와 똑같이 "매 30초 완전히 새로 갱신"(실시간성 전혀 안 바뀜).
    //   2) 그 30초 창 안에서 슬롯을 클릭하면, 방금 그 폴링이 만들어둔 25초짜리
    //      캐시를 재사용 → 사실상 항상 즉시 응답(느려지는 경우는 폴링 직후
    //      25~30초 사이의 짧은 틈뿐이고, 그마저도 원래 속도로 자연스럽게 처리됨).
    //   배치가 매우 크면 100KB 캐시 한도를 넘을 수 있음 — 그럴 땐 캐싱만 건너뜀
    //   (매번 다시 계산되긴 하지만 결과가 틀리진 않음. 기존과 동일한 안전장치).
    if (_payload.length < 95000) _cache.put(_cacheKey, _payload, 25);
...
function saveDimensions(data) {
  ...
  try {
    ...
    bumpVersion_();
    // ★ 세션A 신규 — 디멘션 합산 그룹(linkDimensions로 이 인보이스에 소속된
    //   다른 인보이스들)이 있으면, 이 인보이스의 디멘션 변경이 그 자식 인보이스
    //   전체의 상세조회 결과(dimsCount/movedToPacking 등)에도 영향을 주므로
    //   같이 갱신. 예전엔 이 함수가 salesInvDetail 캐시/미러를 전혀 안
    //   건드려서, 디멘션 저장 직후 상세조회를 열면 최대 6초간 예전 값이
    //   보일 수 있었던 잠복 버그를 여기서 같이 고침. best-effort — 여기서
    //   실패해도 디멘션 저장 자체는 이미 성공한 뒤이므로 무시.
    try {
      const linksMap = buildDimLinksMap_();
      const childInvoices = linksMap.primaryToChildren[invoice] || [];
      syncSalesInvoiceDetailMirror_([invoice].concat(childInvoices));
...
function unlinkDimensions(data) {
  ...
  try {
    const invoice = String((data && data.invoice) || '').trim();
    if (!invoice) return { ok: false, error: 'invoice required' };
    // ★ 세션A 신규 — 행을 지우기 전에 "지금까지 속해 있던 대표"를 먼저
    //   파악해둠. 삭제 후에는 DimLinks에서 이 관계가 사라져서 알아낼 수 없음 —
    //   그리고 그 대표 쪽 상세조회도 이 오더가 빠졌다는 걸 반영해야 함.
    let oldPrimary = invoice;
    try { oldPrimary = resolveDimPrimary_(invoice, buildDimLinksMap_()); } catch (eResolve) { /* best-effort */ }
    const sh = dimLinksSheet_();
    const last = sh.getLastRow();
    let removed = 0;
    if (last >= 2) {
